...
app.post('/register', (req, res) => {
    const { userID, userName, userEmail, userPassword, userPhoneNumber } = req.body;

    if (!userID || !userName || !userEmail || !userPassword || !userPhoneNumber) {
        return res.send('All fields are required!');
    }

    // Insert the new user into the database
    db.query('INSERT INTO user (userId, userName, userEmail, userPassword, userPhoneNumber) VALUES (?, ?, ?, ?, ?)', 
    [userID, userName, userEmail, userPassword, userPhoneNumber], (err, result) => {
        if (err) throw err;
